    ...
    for (int idx = 0; idx < num_frames; idx++) {
        ...
        for (int r = 0; r < frame_height; r++) {
            for (int c = 0; c < frame_width; c++) {
                ...
                uint8_t* centered_pixel = &pad_frame[(r_pad * (frame_width_pad) + c_pad) * colorspace];
                ...
                for (int wi = -radius; wi <= radius; wi++) {
                    for (int wj = -radius; wj <= radius; wj++) {
                        ...
                        uint8_t* nbor_value = &pad_frame[(nbor_i * frame_width_pad + nbor_j) * colorspace];

                        float intensity_dist_sq = vec4_dist_sq_uint8(nbor_value, centered_pixel);

                        float weight = spatial_weights[(wi + radius) * (2 * radius + 1) + (wj + radius)] * fast_gaussian(intensity_dist_sq, intensity_factor);
